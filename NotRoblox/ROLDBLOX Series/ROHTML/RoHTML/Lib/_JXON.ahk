...
; Object() is more technically correct than {} but both will work.
        ...
        if !InStr(next, ch, true) {
            ...
            msg := Format("{}: line {} col {} (char {})"
                ...
                : [ "Expecting JSON value(string, number, [true, false, null], object or array)"
                  , ch := SubStr(src, pos, (SubStr(src, pos)~="[\\\]\},\\s]|$")-1)
                  ][1]
                , ln, col, pos)
...
Jxon_Dump(obj, indent:="", lvl:=1) {
    if IsObject(obj) {
        If !(obj is Array || obj is Map || obj is String || obj is Number)
            throw Error("Object type not supported.", -1, Format("{}", ObjPtr(obj)))
        ...
        for k, v in obj {
            if IsObject(k) || (k == "")
                throw Error("Invalid object key.", -1, k ? Format("{}", ObjPtr(obj)) : "")
            if !is_array ;// key
                ; ObjGetCapacity([k], 1)
                out .= (ObjGetCapacity([k]) ? Jxon_Dump(k) : escape_str(k))
